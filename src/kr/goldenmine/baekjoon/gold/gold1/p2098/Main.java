package kr.goldenmine.baekjoon.gold.gold1.p2098;
...
//    static int totalMin = Integer.MAX_VALUE;
    ...
    // DFS 알고리즘
    private static int dfs(int city, int citySum, int visitBitmask) {

//        if(citySum >= totalMin) return INF;

        if(visitBitmask == (1 << n) - 1) {	// 모든 도시를 방문했다면
//            totalMin = Math.min(totalMin, citySum);
            if(map[city][0] == 0) {	// 마지막 도시에서 0번쨰로 이동할 수 없으면 INF 리턴
                return INF;
            }
        ...
        for(int i = 0; i < n; i++) {	// 현재 도시(city)에서 각 i의 도시로 이동한 경우에 대해 DFS 수행
            if ((visitBitmask & (1 << i)) == (1 << i)) continue;
            if((visitBitmask & (1 << i)) == 0 && map[city][i] != 0) {	// 한 번이라도 그 도시를 방문했는데 다시 그 도시를 방문하는 경우 예외처리
                ...
                dp[city][visitBitmask] = Math.min(dp[city][visitBitmask], dfs(i, citySum + map[city][i], visitBitmask | (1 << i)) + map[city][i]);	// dfs(다음 도시, 다음도시 방문했다고 가정) + 여기서 다음 도시까지의 거리 와 최소거리 비교

//                System.out.println(totalMin + ", " + city + ", " + citySum + ", " + visitBitmask + ", " + map[city][i]);
            }
        }
