package kr.goldenmine.baekjoon.gold.gold5.p14567S;
...
public class Main {
    ...
    static void topologicalSort(int N, int[] indegree, List<List<Integer>> array) throws IOException {
        ...
        while (!q.isEmpty()) {
            Node node = q.poll();
//            result.offer(node.node);

            res[node.node] = node.depth;

            for (Integer i : array.get(node.node)) {
                indegree[i]--;
...
//                    res[i] = Math.max(res[i], )
    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int i = 0; i < M; i++) {
            ...
            indegree[out]++;
            array.get(in).add(out);
        }

//        System.out.println(Arrays.toString(indegree));

        topologicalSort(N, indegree, array);
    }
}
