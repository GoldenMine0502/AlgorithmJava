package kr.goldenmine.contest.c984_div3.E;
...
public class Main {
    ...
    public static int lowerBound(int[][] arr, int k, int key) {
        int lo = 0;
        int hi = arr.length;
    ...
    public static int upperBound(int[][] arr, int k, int key) {
        int lo = 0;
        int hi = arr.length;
...
//            Arrays.fill(maxs, N);
            ...
            for(int i = 0; i < M; i++) {
                ...
                char o = scan.nextChar();
                int c = scan.nextInt();

                if(o == '>') { // r이 c보다 큰 경우
                    mins = Math.max(mins, upperBound(map, r, c));
                } else {
                    maxs = Math.min(maxs, lowerBound(map, r, c));
                }
